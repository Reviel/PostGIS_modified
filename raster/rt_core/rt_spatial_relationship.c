...
rt_errorstate rt_raster_geos_spatial_relationship(
	...
	*testresult = 0;
	...
	GEOSContext_setNoticeHandler_r(lwcontext()->geos_ctx, rtinfo);
	...
	switch (testtype) {
		case GSR_OVERLAPS:
			rtn = GEOSOverlaps_r(lwcontext()->geos_ctx, geom1, geom2);
			break;
		case GSR_TOUCHES:
			rtn = GEOSTouches_r(lwcontext()->geos_ctx, geom1, geom2);
			break;
		case GSR_CONTAINS:
			rtn = GEOSContains_r(lwcontext()->geos_ctx, geom1, geom2);
			break;
		case GSR_CONTAINSPROPERLY:
			rtn = GEOSRelatePattern_r(lwcontext()->geos_ctx, geom1, geom2, "T**FF*FF*");
			break;
		case GSR_COVERS:
			rtn = GEOSRelatePattern_r(lwcontext()->geos_ctx, geom1, geom2, "******FF*");
			break;
		case GSR_COVEREDBY:
			rtn = GEOSRelatePattern_r(lwcontext()->geos_ctx, geom1, geom2, "**F**F***");
			break;
		default:
	...
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, geom1);
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, geom2);
...
rt_raster_intersects(
	...
	do {
		...
		if ((rt_raster_get_convex_hull(rast2, &(hull[1])) != ES_NONE) || !hull[1]) {
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, ghull[0]);
			lwgeom_free(hull[0]);
			break;
		}
		...
		within = 0;
		if (GEOSWithin_r(lwcontext()->geos_ctx, ghull[0], ghull[1]) == 1)
			within = -1;
		else if (GEOSWithin_r(lwcontext()->geos_ctx, ghull[1], ghull[0]) == 1)
			within = 1;

		if (within != 0)
			rtn = 1;
		else
			rtn = GEOSIntersects_r(lwcontext()->geos_ctx, ghull[0], ghull[1]);

		for (i = 0; i < 2; i++) {
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, ghull[i]);
			lwgeom_free(hull[i]);
		}
