	...
	for (x = 0, j = 0, k = 0; x < band->width; x++) {
		...
		for (i = 0, z = 0; i < sample_per; i++) {
			...
			if (rtn == ES_NONE && (!exclude_nodata_value || (exclude_nodata_value && !isnodata))) {
				...
				if (inc_vals)
				{
					/*It is possible that k > sample_size, so memmory needs to be reallocated*/
					if (k >= value_size) {
						value_size *= 2;
						values = rtrealloc(values, value_size * sizeof(double));
					}
					values[k] = value;
				}
...
rt_band_get_histogram(
	...
	/* initialize bins */
	bins = rtalloc((bin_count + bin_width_count) * sizeof(struct rt_histogram_t));
	if (NULL == bins) {
		rterror("rt_util_get_histogram: Could not allocate memory for histogram");
