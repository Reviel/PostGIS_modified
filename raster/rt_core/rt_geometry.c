...
rt_errorstate rt_raster_surface(rt_raster raster, int nband, LWMPOLY **surface) {
	...
	*surface = NULL;
	...
	/* more than 1 polygon */
		...
		gc = GEOSGeom_createCollection_r(lwcontext()->geos_ctx, GEOS_GEOMETRYCOLLECTION, geoms, geomscount);

		if (gc == NULL) {
			rterror("rt_raster_surface: Could not create GEOS GEOMETRYCOLLECTION from set of pixel polygons");

			for (i = 0; i < geomscount; i++)
				GEOSGeom_destroy_r(lwcontext()->geos_ctx, geoms[i]);
			rtdealloc(geoms);
			return ES_ERROR;
		}

		/* run the union */
		gunion = GEOSUnaryUnion_r(lwcontext()->geos_ctx, gc);

		GEOSGeom_destroy_r(lwcontext()->geos_ctx, gc);
		rtdealloc(geoms);
		...
		*/
		do {
			...
			if (GEOSisValid_r(lwcontext()->geos_ctx, gunion))
				break;
		...
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, gunion);
	}
	else {
