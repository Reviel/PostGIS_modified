...
rt_raster_compute_skewed_raster(
	...
	do {
		...
		if ((rt_raster_get_convex_hull(raster, &geom) != ES_NONE) || geom == NULL) {
			...
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, ngeom);
			rt_raster_destroy(raster);
			return NULL;
		}
		...
		covers = GEOSRelatePattern_r(lwcontext()->geos_ctx, sgeom, ngeom, "******FF*");
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, sgeom);
		...
		if (!covers)
		{
			raster->width++;
			raster->height++;
			//raster->width/raster->height --max 65535
			if (raster->width == 65535 || raster->height == 65535)
			{
				rterror("rt_raster_compute_skewed_raster: Could not compute skewed raster,the height or width of the raster in the compute is greater than 65535");
				GEOSGeom_destroy_r(lwcontext()->geos_ctx, ngeom);
				rt_raster_destroy(raster);
				return NULL;
			}
...
//	rt_util_gdal_register_all(0);
	...
	//Used to didtinguish threads
	context = (uint64_t)lwcontext();

	sprintf(dat, "%s%I64x%s", "/vsimem/out", context,".dat");
	...
	rtn_ds = GDALCreateCopy(
		rtn_drv,
		dat, /* should be fine assuming this is in a process */
		src_ds,
	...
	rtn = VSIGetMemFileBuffer(dat, &rtn_lenvsi, TRUE);
