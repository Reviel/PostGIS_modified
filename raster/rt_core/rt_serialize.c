...
rt_raster_deserialize(void* serialized, int header_only) {
	...
	for (i = 0; i < rast->numBands; ++i) {
		...
		if (band->offline) {
			...
			band->data.mem = rtalloc(datasize);
			if (!band->data.mem && datasize != 0) {
				rterror("rt_raster_deserialize: Out of memory during band creation in raster deserialize");
				rt_band_destroy(band);
				return NULL;
			}

			band->ownsdata = 1; /* we DO own this data!!! */
			memcpy(band->data.mem, ptr, datasize);
			ptr += datasize;
		}
