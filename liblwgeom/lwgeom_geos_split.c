...
lwline_split_by_line(const LWLINE* lwline_in, const LWGEOM* blade_in)
{
	...
	{
		gdiff = GEOSBoundary_r(lwcontext()->geos_ctx, g2);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
		if ( ! gdiff )
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
	...
	ret = GEOSRelatePattern_r(lwcontext()->geos_ctx, g1, g2, "1********");
	if ( 2 == ret )
	{
	...
	gdiff = GEOSDifference_r(lwcontext()->geos_ctx, g1,g2);
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
	if (gdiff == NULL)
	{
	...
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, gdiff);
	if (NULL == diff)
	{
...
lwpoly_split_by_line(const LWPOLY* lwpoly_in, const LWGEOM* blade_in)
{
	...
	g1_bounds = GEOSBoundary_r(lwcontext()->geos_ctx, g1);
	if ( NULL == g1_bounds )
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
		lwerror("GEOSBoundary: %s", lwgeom_geos_errmsg);
	...
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1_bounds);
		lwerror("LWGEOM2GEOS: %s", lwgeom_geos_errmsg);
		return NULL;
	}

	vgeoms[0] = GEOSUnion_r(lwcontext()->geos_ctx, g1_bounds, g2);
	if ( NULL == vgeoms[0] )
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
	...
	polygons = GEOSPolygonize_r(lwcontext()->geos_ctx, vgeoms, 1);
	if ( NULL == polygons )
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1_bounds);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, (GEOSGeometry*)vgeoms[0]);
		lwerror("GEOSPolygonize: %s", lwgeom_geos_errmsg);
		return NULL;
	}

#if PARANOIA_LEVEL > 0
	if ( GEOSGeomTypeId_r(lwcontext()->geos_ctx, polygons) != COLLECTIONTYPE )
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1_bounds);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, (GEOSGeometry*)vgeoms[0]);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, polygons);
	 ...
	 */
	n = GEOSGetNumGeometries_r(lwcontext()->geos_ctx, polygons);
	...
	for (i=0; i<n; ++i)
	{
		...
		const GEOSGeometry* p = GEOSGetGeometryN_r(lwcontext()->geos_ctx, polygons, i);
		int contains;

		pos = GEOSPointOnSurface_r(lwcontext()->geos_ctx, p);
		if ( ! pos )
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
		...
		contains = GEOSContains_r(lwcontext()->geos_ctx, g1, pos);
		if ( 2 == contains )
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g1);
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g2);
			...
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, (GEOSGeometry*)vgeoms[0]);
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, polygons);
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, pos);
