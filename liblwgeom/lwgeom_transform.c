...
lwproj_from_str(const char* str_in, const char* str_out)
{
	...
	PJ* pj = proj_create_crs_to_crs(lwcontext()->pj_ctx, str_in, str_out, NULL);
	if (!pj)
		return NULL;
	...
	{
		PJ *pj_source_crs = proj_get_source_crs(lwcontext()->pj_ctx, pj);
		PJ *pj_ellps;
		...
		pj_ellps = proj_get_ellipsoid(lwcontext()->pj_ctx, pj_source_crs);
		...
		if (!proj_ellipsoid_get_parameters(lwcontext()->pj_ctx,
						   pj_ellps,
						   &semi_major_metre,
						   &semi_minor_metre,
						   NULL,
	...
	PJ* pj_norm = proj_normalize_for_visualization(lwcontext()->pj_ctx, pj);
...
lwproj_from_str_pipeline(const char* str_pipeline, bool is_forward)
{
	...
	PJ* pj = proj_create(lwcontext()->pj_ctx, str_pipeline);
	if (!pj)
		return NULL;
...
lwgeom_transform_from_str(LWGEOM *geom, const char* instr, const char* outstr)
{
	...
	if (!lp)
	{
		PJ *pj_in = proj_create(lwcontext()->pj_ctx, instr);
		if (!pj_in)
		{
 			proj_context_errno_set(lwcontext()->pj_ctx, 0);
			lwerror("could not parse proj string '%s'", instr);
		}
		proj_destroy(pj_in);

		PJ *pj_out = proj_create(lwcontext()->pj_ctx, outstr);
		if (!pj_out)
		{
			proj_context_errno_set(lwcontext()->pj_ctx, 0);
...
lwgeom_transform_pipeline(LWGEOM *geom, const char* pipelinestr, bool is_forward)
{
	...
	if (!lp)
	{
		PJ *pj_in = proj_create(lwcontext()->pj_ctx, pipelinestr);
		if (!pj_in)
		{
			proj_context_errno_set(lwcontext()->pj_ctx, 0);
