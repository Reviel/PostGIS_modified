...
ptarray_to_SFCGAL(const POINTARRAY *pa, int type)
{
	...
	case LINETYPE:
	{
		...
		sfcgal_geometry_t *point_sfcgal = NULL;
		for (i = 0; i < pa->npoints; i++)
		{
			getPoint3dz_p(pa, i, &point);
			if (is_3d)
			{
				point_sfcgal = sfcgal_point_create_from_xyz(point.x, point.y, point.z);
				if (!point_sfcgal) 
				{
					sfcgal_geometry_delete(line);
					return NULL;
				}
				sfcgal_linestring_add_point(line, point_sfcgal);
			}
			else
			{
				point_sfcgal = sfcgal_point_create_from_xy(point.x, point.y);
				if (!point_sfcgal)
				{
					sfcgal_geometry_delete(line);
					return NULL;
	...
	case TRIANGLETYPE:
	{
		sfcgal_geometry_t *triangle = NULL;
		sfcgal_geometry_t *pta = NULL;
		sfcgal_geometry_t *ptb = NULL;
		sfcgal_geometry_t *ptc = NULL;
		...
		triangle = sfcgal_triangle_create_from_points(pta, ptb, ptc);
		if (!triangle)
		{
			sfcgal_geometry_delete(pta);
			sfcgal_geometry_delete(ptb);
...
LWGEOM2SFCGAL(const LWGEOM *geom)
{
	...
	case POLYGONTYPE:
	{
		...
		if (!ret_geom)
		{
			sfcgal_geometry_delete(exterior_ring);
			return NULL;
		}
		...
		for (i = 0; i < nrings; i++)
		{
			...
			if (!ring)
			{
				sfcgal_geometry_delete(ret_geom);
				return NULL;
			}
