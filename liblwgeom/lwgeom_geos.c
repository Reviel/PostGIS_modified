...
static void geos_destroy(size_t count, ...) {
	...
	while (count--)
	{
		...
		if (g)
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, g);
		}
	}
	va_end(ap);
}
...
ptarray_from_GEOSCoordSeq(const GEOSCoordSequence* cs, uint8_t want3d)
{
	...
	if (!GEOSCoordSeq_getSize_r(lwcontext()->geos_ctx, cs, &size)) lwerror("Exception thrown");

	LWDEBUGF(4, " GEOSCoordSeq size: %d", size);

	if (want3d)
	{
		if (!GEOSCoordSeq_getDimensions_r(lwcontext()->geos_ctx, cs, &dims)) lwerror("Exception thrown");
...
#if POSTGIS_GEOS_VERSION >= 31000
	GEOSCoordSeq_copyToBuffer_r(lwcontext()->geos_ctx, cs, (double*) pa->serialized_pointlist, (dims == 3), 0);
	return pa;
#else
	for (i = 0; i < size; i++)
	{
#if POSTGIS_GEOS_VERSION < 30800
		GEOSCoordSeq_getX_r(lwcontext()->geos_ctx, cs, i, &(point.x));
		GEOSCoordSeq_getY_r(lwcontext()->geos_ctx, cs, i, &(point.y));
		if (dims >= 3) GEOSCoordSeq_getZ_r(lwcontext()->geos_ctx, cs, i, &(point.z));
#else
		if (dims >= 3)
			GEOSCoordSeq_getXYZ_r(lwcontext()->geos_ctx, cs, i, &(point.x), &(point.y), &(point.z));
		else
			GEOSCoordSeq_getXY_r(lwcontext()->geos_ctx, cs, i, &(point.x), &(point.y));
#endif
...
{
	int type = GEOSGeomTypeId_r(lwcontext()->geos_ctx, geom);
	int SRID = GEOSGetSRID_r(lwcontext()->geos_ctx, geom);

	/* GEOS's 0 is equivalent to our unknown as for SRID values */
	if (SRID == 0) SRID = SRID_UNKNOWN;

	if (want3d && !GEOSHasZ_r(lwcontext()->geos_ctx, geom))
	{
	...
	case GEOS_POINT:
		...
		cs = GEOSGeom_getCoordSeq_r(lwcontext()->geos_ctx, geom);
		if (GEOSisEmpty_r(lwcontext()->geos_ctx, geom)) return (LWGEOM*)lwpoint_construct_empty(SRID, want3d, 0);
	...
	case GEOS_LINEARRING:
		...
		if (GEOSisEmpty_r(lwcontext()->geos_ctx ,geom)) return (LWGEOM*)lwline_construct_empty(SRID, want3d, 0);
	...
	case GEOS_POLYGON:
		...
		if (GEOSisEmpty_r(lwcontext()->geos_ctx, geom)) return (LWGEOM*)lwpoly_construct_empty(SRID, want3d, 0);
		ngeoms = GEOSGetNumInteriorRings_r(lwcontext()->geos_ctx, geom);
		ppaa = lwalloc(sizeof(POINTARRAY*) * (ngeoms + 1));
		g = GEOSGetExteriorRing_r(lwcontext()->geos_ctx, geom);
		cs = GEOSGeom_getCoordSeq_r(lwcontext()->geos_ctx, g);
		ppaa[0] = ptarray_from_GEOSCoordSeq(cs, want3d);
		for (i = 0; i < ngeoms; i++)
		{
			g = GEOSGetInteriorRingN_r(lwcontext()->geos_ctx, geom, i);
			cs = GEOSGeom_getCoordSeq_r(lwcontext()->geos_ctx, g);
	...
	case GEOS_GEOMETRYCOLLECTION:
		...
		ngeoms = GEOSGetNumGeometries_r(lwcontext()->geos_ctx, geom);
		geoms = NULL;
		if (ngeoms)
		{
			...
			for (i = 0; i < ngeoms; i++)
			{
				g = GEOSGetGeometryN_r(lwcontext()->geos_ctx, geom, i);
...
#if POSTGIS_GEOS_VERSION >= 31000
	if (append_points == 0) {
		sq = GEOSCoordSeq_copyFromBuffer_r(lwcontext()->geos_ctx, (const double*) pa->serialized_pointlist, pa->npoints, FLAGS_GET_Z(pa->flags), FLAGS_GET_M(pa->flags));
		if (!sq)
		{
			GEOS_FAIL();
		}
...
#endif
        if (!(sq = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, pa->npoints + append_points, dims)))
	{
		GEOS_FAIL();
	}
...
#if POSTGIS_GEOS_VERSION < 30800
		GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, sq, i, p2d->x);
		GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, sq, i, p2d->y);
		if (dims == 3) GEOSCoordSeq_setZ_r(lwcontext()->geos_ctx, sq, i, p3d->z);
#else
		if (dims == 3)
			GEOSCoordSeq_setXYZ_r(lwcontext()->geos_ctx, sq, i, p2d->x, p2d->y, p3d->z);
		else
			GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, sq, i, p2d->x, p2d->y);
#endif
...
ptarray_to_GEOSLinearRing(const POINTARRAY* pa, uint8_t autofix)
{
	...
	g = GEOSGeom_createLinearRing_r(lwcontext()->geos_ctx, sq);
	return g;
}
...
GBOX2GEOS(const GBOX* box)
{
	...
	GEOSGeometry* ring;
	GEOSCoordSequence* seq = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, 5, 2);
	if (!seq) return NULL;

#if POSTGIS_GEOS_VERSION < 30800
	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 0, box->xmin);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 0, box->ymin);

	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 1, box->xmax);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 1, box->ymin);

	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 2, box->xmax);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 2, box->ymax);

	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 3, box->xmin);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 3, box->ymax);

	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 4, box->xmin);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 4, box->ymin);
#else
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 0, box->xmin, box->ymin);
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 1, box->xmax, box->ymin);
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 2, box->xmax, box->ymax);
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 3, box->xmin, box->ymax);
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 4, box->xmin, box->ymin);
#endif

	ring = GEOSGeom_createLinearRing_r(lwcontext()->geos_ctx, seq);
	if (!ring)
	{
		GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, seq);
		return NULL;
	}

	envelope = GEOSGeom_createPolygon_r(lwcontext()->geos_ctx, ring, NULL, 0);
	if (!envelope)
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, ring);
		return NULL;
	}
...
LWGEOM2GEOS(const LWGEOM* lwgeom, uint8_t autofix)
{
	...
	switch (lwgeom->type)
	{
		...
		{
			if (is_empty)
				g = GEOSGeom_createEmptyPoint_r(lwcontext()->geos_ctx);
			else
			{
				LWPOINT* lwp = (LWPOINT*)lwgeom;
	#if POSTGIS_GEOS_VERSION < 30800
				sq = ptarray_to_GEOSCoordSeq(lwp->point, 0);
				g = GEOSGeom_createPoint_r(lwcontext()->geos_ctx, sq);
	#else
				...
				else
				{
					...
					g = GEOSGeom_createPointFromXY_r(lwcontext()->geos_ctx, p->x, p->y);
				}
	#endif
			}
			if (!g) return NULL;
	...
	case LINETYPE:
		{
			if (is_empty)
				g = GEOSGeom_createEmptyLineString_r(lwcontext()->geos_ctx);
			else
			{
				...
				g = GEOSGeom_createLineString_r(lwcontext()->geos_ctx, sq);
			}
			if (!g) return NULL;
			break;
		}
		...
		case POLYGONTYPE:
		{
			...
			if (is_empty)
				g = GEOSGeom_createEmptyPolygon_r(lwcontext()->geos_ctx);
			else
			{
				...
				for (i = 1; i < lwpoly->nrings; i++)
				{
					...
					if (!geoms[i - 1])
					{
						uint32_t k;
						for (k = 0; k < i - 1; k++)
							GEOSGeom_destroy_r(lwcontext()->geos_ctx, geoms[k]);
						lwfree(geoms);
						GEOSGeom_destroy_r(lwcontext()->geos_ctx, shell);
						return NULL;
					}
				}
				g = GEOSGeom_createPolygon_r(lwcontext()->geos_ctx, shell, geoms, ngeoms);
		...
		case TRIANGLETYPE:
		{
			...
			else
			{
				...
				if (!shell) return NULL;
				g = GEOSGeom_createPolygon_r(lwcontext()->geos_ctx, shell, NULL, 0);
			}
			if (!g) return NULL;
			break;
		}
		...
		case COLLECTIONTYPE:
		{
			...
			g = GEOSGeom_createCollection_r(lwcontext()->geos_ctx, geostype, geoms, j);
	...
	GEOSSetSRID_r(lwcontext()->geos_ctx, g, lwgeom->srid);

#if LWDEBUG_LEVEL >= 4
	wkt = GEOSGeomToWKT_r(lwcontext()->geos_ctx, g);
	LWDEBUGF(4, "LWGEOM2GEOS: GEOSGeom: %s", wkt);
	GEOSFree_r(lwcontext()->geos_ctx, wkt);
#endif
...
{
	GEOSCoordSequence* seq = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, 1, 2);
	GEOSGeometry* geom = NULL;

	if (!seq) return NULL;

#if POSTGIS_GEOS_VERSION < 30800
	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 0, x);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 0, y);
#else
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 0, x, y);
#endif

	geom = GEOSGeom_createPoint_r(lwcontext()->geos_ctx, seq);
	if (!geom) GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, seq);
	return geom;
}
...
{
	GEOSCoordSequence* seq = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, 2, 2);
	GEOSGeometry* geom = NULL;

	if (!seq) return NULL;

#if POSTGIS_GEOS_VERSION < 30800
	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 0, x1);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 0, y1);
	GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, seq, 1, x2);
	GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, seq, 1, y2);
#else
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 0, x1, y1);
	GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, seq, 1, x2, y2);
#endif

	geom = GEOSGeom_createLineString_r(lwcontext()->geos_ctx, seq);
	if (!geom) GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, seq);
	return geom;
}
...
lwgeom_normalize(const LWGEOM* geom)
{
	...
	if (GEOSNormalize_r(lwcontext()->geos_ctx, g) == -1) GEOS_FREE_AND_FAIL(g);
	GEOSSetSRID_r(lwcontext()->geos_ctx, g, srid);
...
lwgeom_intersection_prec(const LWGEOM* geom1, const LWGEOM* geom2, double prec)
{
	...
	if ( prec >= 0) {
		...
		return NULL;
#else
		g3 = GEOSIntersectionPrec_r(lwcontext()->geos_ctx, g1, g2, prec);
#endif
	}
	else
	{
		g3 = GEOSIntersection_r(lwcontext()->geos_ctx, g1, g2);
	}

	if (!g3) GEOS_FREE_AND_FAIL(g1);
	GEOSSetSRID_r(lwcontext()->geos_ctx, g3, srid);
...
lwgeom_linemerge_directed(const LWGEOM* geom, int directed)
{
	...
	if (directed)
	{
		...
		return NULL;
#else
		g3 = GEOSLineMergeDirected_r(lwcontext()->geos_ctx, g1);
#endif
	}
	else
	{
		g3 = GEOSLineMerge_r(lwcontext()->geos_ctx, g1);
	}
...
lwgeom_unaryunion_prec(const LWGEOM* geom, double prec)
{
	...
	if ( prec >= 0) {
		...
		return NULL;
#else
		g3 = GEOSUnaryUnionPrec_r(lwcontext()->geos_ctx, g1, prec);
#endif
	}
	else
	{
		g3 = GEOSUnaryUnion_r(lwcontext()->geos_ctx, g1);
	}
...
lwgeom_difference_prec(const LWGEOM* geom1, const LWGEOM* geom2, double prec)
{
	...
	if ( prec >= 0) {
		...
		return NULL;
#else
		g3 = GEOSDifferencePrec_r(lwcontext()->geos_ctx, g1, g2, prec);
#endif
	}
	else
	{
		g3 = GEOSDifference_r(lwcontext()->geos_ctx, g1, g2);
	}
...
lwgeom_symdifference_prec(const LWGEOM* geom1, const LWGEOM* geom2, double prec)
{
	...
	if ( prec >= 0) {
		...
		return NULL;
#else
		g3 = GEOSSymDifferencePrec_r(lwcontext()->geos_ctx, g1, g2, prec);
#endif
	}
	else
	{
		g3 = GEOSSymDifference_r(lwcontext()->geos_ctx, g1, g2);
	}
...
lwgeom_centroid(const LWGEOM* geom)
{
	...
	g3 = GEOSGetCentroid_r(lwcontext()->geos_ctx, g1);
...
lwgeom_reduceprecision(const LWGEOM* geom, double gridSize)
{
	...
	g3 = GEOSGeom_setPrecision_r(lwcontext()->geos_ctx, g1, gridSize, 0);
...
lwgeom_pointonsurface(const LWGEOM *geom)
{
	...
	g3 = GEOSPointOnSurface_r(lwcontext()->geos_ctx, g1);
...
lwgeom_union_prec(const LWGEOM* geom1, const LWGEOM* geom2, double gridSize)
{
	...
	if ( gridSize >= 0) {
		...
		return NULL;
#else
		g3 = GEOSUnionPrec_r(lwcontext()->geos_ctx, g1, g2, gridSize);
#endif
	}
	else
	{
		g3 = GEOSUnion_r(lwcontext()->geos_ctx, g1, g2);
	}
...
lwgeom_clip_by_rect(const LWGEOM *geom1, double x1, double y1, double x2, double y2)
{
	...
	if (!(g3 = GEOSClipByRect_r(lwcontext()->geos_ctx, g1, x1, y1, x2, y2)))
...
newFace(const GEOSGeometry* g)
{
	...
	f->geom = g;
	f->env = GEOSEnvelope_r(lwcontext()->geos_ctx, f->geom);
	GEOSArea_r(lwcontext()->geos_ctx, f->env, &f->envarea);
	f->parent = NULL;
	return f;
}
...
delFace(Face* f)
{
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, f->env);
	lwfree(f);
}
...
findFaceHoles(Face** faces, int nfaces)
{
	...
	{
		Face* f = faces[i];
		int nholes = GEOSGetNumInteriorRings_r(lwcontext()->geos_ctx, f->geom);
		LWDEBUGF(2, "Scanning face %d with env area %g and %d holes", i, f->envarea, nholes);
		for (h = 0; h < nholes; ++h)
		{
			const GEOSGeometry* hole = GEOSGetInteriorRingN_r(lwcontext()->geos_ctx, f->geom, h);
			LWDEBUGF(2,
			...
			for (j = i + 1; j < nfaces; ++j)
			{
				...
				f2er = GEOSGetExteriorRing_r(lwcontext()->geos_ctx, f2->geom);
				...
				if (GEOSEquals_r(lwcontext()->geos_ctx, f2er, hole))
				{
...
collectFacesWithEvenAncestors(Face** faces, int nfaces)
{
	...
	for (i = 0; i < nfaces; ++i)
	{
		...
		geoms[ngeoms++] = GEOSGeom_clone_r(lwcontext()->geos_ctx, f->geom);
	}

	ret = GEOSGeom_createCollection_r(lwcontext()->geos_ctx, GEOS_MULTIPOLYGON, geoms, ngeoms);
	lwfree(geoms);
	return ret;
}
...
LWGEOM_GEOS_buildArea(const GEOSGeometry* geom_in)
{
	...
	uint32_t i, ngeoms;
	int srid = GEOSGetSRID_r(lwcontext()->geos_ctx, geom_in);
	Face** geoms;
	...
	vgeoms[0] = geom_in;
	geos_result = GEOSPolygonize_r(lwcontext()->geos_ctx, vgeoms, 1);
...
#if PARANOIA_LEVEL > 0
	if (GEOSGeomTypeId_r(lwcontext()->geos_ctx, geos_result) != COLLECTIONTYPE)
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, geos_result);
		lwerror("%s [%d] Unexpected return from GEOSpolygonize", __FILE__, __LINE__);
		return 0;
	}
#endif

	ngeoms = GEOSGetNumGeometries_r(lwcontext()->geos_ctx, geos_result);
	...
	if (ngeoms == 0)
	{
		GEOSSetSRID_r(lwcontext()->geos_ctx, geos_result, srid);
		return geos_result;
	}
	...
	if (ngeoms == 1)
	{
		tmp = (GEOSGeometry*)GEOSGetGeometryN_r(lwcontext()->geos_ctx, geos_result, 0);
		if (!tmp)
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, geos_result);
			return 0; /* exception */
		}
		shp = GEOSGeom_clone_r(lwcontext()->geos_ctx, tmp);
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, geos_result); /* only safe after the clone above */
		GEOSSetSRID_r(lwcontext()->geos_ctx, shp, srid);
		return shp;
	}
	...
	for (i = 0; i < ngeoms; ++i)
		geoms[i] = newFace(GEOSGetGeometryN_r(lwcontext()->geos_ctx, geos_result, i));
	...
	shp = GEOSUnionCascaded_r(lwcontext()->geos_ctx, tmp);
	if (!shp)
	{
		GEOSGeom_destroy_r(lwcontext()->geos_ctx, tmp);
		return 0; /* exception */
	}
...
lwgeom_buildarea(const LWGEOM* geom)
{
...
#else
	g3 = GEOSBuildArea_r(lwcontext()->geos_ctx, g1);
#endif
	...
	if (GEOSGetNumGeometries_r(lwcontext()->geos_ctx, g3) == 0)
	{
		GEOS_FREE(g1);
		return NULL;
	}
...
lwgeom_is_simple(const LWGEOM* geom)
{
	...
	simple = GEOSisSimple_r(lwcontext()->geos_ctx, g);
	GEOSGeom_destroy_r(lwcontext()->geos_ctx, g);
...
lwgeom_snap(const LWGEOM* geom1, const LWGEOM* geom2, double tolerance)
{
	...
	g3 = GEOSSnap_r(lwcontext()->geos_ctx, g1, g2, tolerance);
...
lwgeom_sharedpaths(const LWGEOM* geom1, const LWGEOM* geom2)
{
	...
	g3 = GEOSSharedPaths_r(lwcontext()->geos_ctx, g1, g2);
...
lwline_offsetcurve(const LWLINE *lwline, double size, int quadsegs, int joinStyle, double mitreLimit)
{
	...
	g3 = GEOSOffsetCurve_r(lwcontext()->geos_ctx, g1, size, quadsegs, joinStyle, mitreLimit);
...
lwpoly_to_points(const LWPOLY* lwpoly, uint32_t npoints, int32_t seed)
{
	...
	gprep = GEOSPrepare_r(lwcontext()->geos_ctx, g);
	...
	while (npoints_generated < npoints)
	{
		...
		for (i = 0; i < sample_width * sample_height; i++)
		{
			...
			gseq = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, 1, 2);
#if POSTGIS_GEOS_VERSION < 30800
			GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, gseq, 0, x);
			GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, gseq, 0, y);
#else
			GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, gseq, 0, x, y);
#endif
			gpt = GEOSGeom_createPoint_r(lwcontext()->geos_ctx, gseq);

			contains = GEOSPreparedIntersects_r(lwcontext()->geos_ctx, gprep, gpt);

			GEOSGeom_destroy_r(lwcontext()->geos_ctx, gpt);

			if (contains == 2)
			{
				GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, gprep);
				GEOSGeom_destroy_r(lwcontext()->geos_ctx, g);
			...
			if (npoints_tested % 10000 == 0)
				LW_ON_INTERRUPT(GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, gprep); GEOSGeom_destroy_r(lwcontext()->geos_ctx, g); return NULL);
	...
	case GEOS_GEOMETRYCOLLECTION:
		...
		if (ngeoms)
		{
			...
			for (i = 0; i < ngeoms; i++)
			{
				...
				poly = GEOSGetGeometryN_r(lwcontext()->geos_ctx, geom, i);
				ring = GEOSGetExteriorRing_r(lwcontext()->geos_ctx, poly);
				cs = GEOSGeom_getCoordSeq_r(lwcontext()->geos_ctx, ring);
				pa = ptarray_from_GEOSCoordSeq(cs, want3d);
...
lwgeom_delaunay_triangulation(const LWGEOM* geom, double tolerance, int32_t output)
{
	...
	g3 = GEOSDelaunayTriangulation_r(lwcontext()->geos_ctx, g1, tolerance, output == 1);
...
lwgeom_get_geos_coordseq_2d(const LWGEOM* g, uint32_t num_points)
{
	...
	coords = GEOSCoordSeq_create_r(lwcontext()->geos_ctx, num_points, num_dims);
	...
	while (lwpointiterator_next(it, &tmp))
	{
		if (i >= num_points)
		{
			...
			GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, coords);
			lwpointiterator_destroy(it);
			return NULL;
		}

#if POSTGIS_GEOS_VERSION < 30800
		if (!GEOSCoordSeq_setX_r(lwcontext()->geos_ctx, coords, i, tmp.x) || !GEOSCoordSeq_setY_r(lwcontext()->geos_ctx, coords, i, tmp.y))
#else
		if (!GEOSCoordSeq_setXY_r(lwcontext()->geos_ctx, coords, i, tmp.x, tmp.y))
#endif
		{
			GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, coords);
			lwpointiterator_destroy(it);
...
lwgeom_voronoi_diagram(const LWGEOM* g, const GBOX* env, double tolerance, int output_edges)
{
	...
	geos_geom = GEOSGeom_createLineString_r(lwcontext()->geos_ctx, coords);
	if (!geos_geom)
	{
		GEOSCoordSeq_destroy_r(lwcontext()->geos_ctx, coords);
		return NULL;
	...
	geos_result = GEOSVoronoiDiagram_r(lwcontext()->geos_ctx, geos_geom, geos_env, tolerance, output_edges);

	GEOSGeom_destroy_r(lwcontext()->geos_ctx, geos_geom);
	if (env) GEOSGeom_destroy_r(lwcontext()->geos_ctx, geos_env);
...
lwgeom_concavehull(const LWGEOM* geom, double ratio, uint32_t allow_holes)
{
	...
	geosGeomType = GEOSGeomTypeId_r(lwcontext()->geos_ctx, g1);
	if (geosGeomType == GEOS_POLYGON || geosGeomType == GEOS_MULTIPOLYGON) {
		int is_tight = LW_FALSE;
		g3 = GEOSConcaveHullOfPolygons_r(lwcontext()->geos_ctx, g1, ratio, is_tight, allow_holes);
	}
	else {
		g3 = GEOSConcaveHull_r(lwcontext()->geos_ctx, g1, ratio, allow_holes);
	}
...
lwgeom_simplify_polygonal(const LWGEOM* geom, double vertex_fraction, uint32_t is_outer)
{
	...
	g3 = GEOSPolygonHullSimplify_r(lwcontext()->geos_ctx, g1, is_outer, vertex_fraction);
...
lwgeom_triangulate_polygon(const LWGEOM* geom)
{
	...
	/* if output != 1 we want polys */
	g3 = GEOSConstrainedDelaunayTriangulation_r(lwcontext()->geos_ctx, g1);
