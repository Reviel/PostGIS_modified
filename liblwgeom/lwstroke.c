                 ...
                 int flags)
{
	...
	/* The statement above will produce different results when executed on x86 and ARM machines.On x86, the
	 * result is a negative number (overflow), while on ARM, the result is the maximum positive value which
	 * is not expected. Therefore, I add a conditional statement on the blow statement. */
	if (segments < min_segs || segments >= 2147483647)
	{
		segments = min_segs;
