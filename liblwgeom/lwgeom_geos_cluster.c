...
geos_envelope_surrogate(const LWGEOM* g)
{
	if (lwgeom_is_empty(g))
		return GEOSGeom_createEmptyPolygon_r(lwcontext()->geos_ctx);
...
make_strtree(void** geoms, uint32_t num_geoms, char is_lwgeom)
{
	...
	tree.tree = GEOSSTRtree_create_r(lwcontext()->geos_ctx, STRTREE_NODE_CAPACITY);
	if (tree.tree == NULL)
	{
		return tree;
	}
	...
	if (is_lwgeom)
	{
		...
		for (i = 0; i < num_geoms; i++)
		{
			...
			GEOSSTRtree_insert_r(lwcontext()->geos_ctx, tree.tree, tree.envelopes[i], &(tree.geom_ids[i]));
		}
	}
	else
	{
		...
		{
			tree.geom_ids[i] = i;
			GEOSSTRtree_insert_r(lwcontext()->geos_ctx, tree.tree, geoms[i], &(tree.geom_ids[i]));
		}
	}
...
{
	size_t i;
	GEOSSTRtree_destroy_r(lwcontext()->geos_ctx, tree->tree);

	if (tree->envelopes)
	{
		for (i = 0; i < tree->num_geoms; i++)
		{
			GEOSGeom_destroy_r(lwcontext()->geos_ctx, tree->envelopes[i]);
		}
...
union_intersecting_pairs(GEOSGeometry** geoms, uint32_t num_geoms, UNIONFIND* uf)
{
	...
	for (p = 0; p < num_geoms; p++)
	{
		...
		if (!geoms[p] || GEOSisEmpty_r(lwcontext()->geos_ctx, geoms[p]))
			continue;

		cxt.num_items_found = 0;
		GEOSSTRtree_query_r(lwcontext()->geos_ctx, tree.tree, geoms[p], &query_accumulate, &cxt);
		...
		for (i = 0; i < cxt.num_items_found; i++)
		{
			...
			{
				int geos_type = GEOSGeomTypeId_r(lwcontext()->geos_ctx, geoms[p]);
				int geos_result;
				...
				if (geos_type != GEOS_POINT && geos_type != GEOS_MULTIPOINT)
				{
					...
					if (prep == NULL)
					{
						prep = GEOSPrepare_r(lwcontext()->geos_ctx, geoms[p]);
					}
					geos_result = GEOSPreparedIntersects_r(lwcontext()->geos_ctx, prep, geoms[q]);
				}
				else
				{
					geos_result = GEOSIntersects_r(lwcontext()->geos_ctx, geoms[p], geoms[q]);
				}
				if (geos_result > 1)
				{
					success = LW_FAILURE;
		...
		if (prep)
			GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, prep);
...
dbscan_update_context(GEOSSTRtree* tree, struct QueryContext* cxt, LWGEOM** geoms, uint32_t p, double eps)
{
	...
	GEOSSTRtree_query_r(lwcontext()->geos_ctx, tree, query_envelope, &query_accumulate, cxt);

	GEOSGeom_destroy_r(lwcontext()->geos_ctx, query_envelope);
	...
	*clusterGeoms = lwalloc(*num_clusters * sizeof(void*));
	...
	for (i = 0, j = 0, k = 0; i < num_geoms; i++)
	{
		        ...
		        (UF_find(uf, ordered_components[i]) != UF_find(uf, ordered_components[i+1])))
		{
			...
			{
				int srid = GEOSGetSRID_r(lwcontext()->geos_ctx, ((GEOSGeometry**) geoms_in_cluster)[0]);
				GEOSGeometry* combined = GEOSGeom_createCollection_r(lwcontext()->geos_ctx, GEOS_GEOMETRYCOLLECTION, (GEOSGeometry**) geoms_in_cluster, j);
				GEOSSetSRID_r(lwcontext()->geos_ctx, combined, srid);
