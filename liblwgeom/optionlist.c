...
option_list_parse(char* input, char** olist)
{
	...
	for (key = strtok_sr(input, toksep, &outer_ptr); key; key = strtok_sr(NULL, toksep, &outer_ptr)) {
...
int
option_list_geos_parse(char* input, char** olist)
{
	const char* toksep = " ";
	const char kvsep = '=';
	char* key, * val;
	...
	for (i = 0; i < sz; i += 2) {
		...
		val = strchr(key, kvsep);
		if (!val) {
			lwnotice("Option string entry '%s' lacks separator '%c'", key, kvsep);
			return 0;
		}
		/* null out the separator */
		*val = '\0';
