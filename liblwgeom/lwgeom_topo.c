                        ...
                        const LWLINE *geom, LWT_ELEMID myself )
{
  ...
  for ( i=0; i<num_nodes; ++i )
  {
    ...
    if ( contains )
    {
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
  ...
  for ( i=0; i<num_edges; ++i )
  {
    ...
    relate = GEOSRelateBoundaryNodeRule_r(lwcontext()->geos_ctx, eegg, edgegg, 2);
    if ( ! relate ) {
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, eegg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
      _lwt_release_edges(edges, num_edges);
    ...
    match = GEOSRelatePatternMatch_r(lwcontext()->geos_ctx, relate, "F********");
    if ( match ) {
      /* error or no interior intersection */
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, eegg);
      GEOSFree_r(lwcontext()->geos_ctx, relate);
      if ( match == 2 ) {
        _lwt_release_edges(edges, num_edges);
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
    ...
    match = GEOSRelatePatternMatch_r(lwcontext()->geos_ctx, relate, "1FFF*FFF2");
    if ( match ) {
      _lwt_release_edges(edges, num_edges);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, eegg);
    ...
    match = GEOSRelatePatternMatch_r(lwcontext()->geos_ctx, relate, "1********");
    if ( match ) {
      _lwt_release_edges(edges, num_edges);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, eegg);
    ...
    match = GEOSRelatePatternMatch_r(lwcontext()->geos_ctx, relate, "T********");
    if ( match ) {
      _lwt_release_edges(edges, num_edges);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgegg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, eegg);
...
_lwt_GetEqualEdge( LWT_TOPOLOGY *topo, LWLINE *edge, int *forward )
{
  ...
  if ( num )
  {
    ...
    for (i=0; i<num; ++i)
    {
      ...
      if ( ! gg )
      {
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgeg);
        _lwt_release_edges(edges, num);
        lwerror("Could not convert edge geometry to GEOS: %s", lwgeom_geos_errmsg);
        return -1;
      }
      equals = GEOSEquals_r(lwcontext()->geos_ctx, gg, edgeg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, gg);
      if ( equals == 2 )
      {
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, edgeg);
        _lwt_release_edges(edges, num);
...
lwt_AddPolygon(LWT_TOPOLOGY* topo, LWPOLY* poly, double tol, int* nfaces)
{
  ...
  */
  ...
  if ( nfacesinbox )
  {
    ...
    ppoly = GEOSPrepare_r(lwcontext()->geos_ctx, polyg);
    ...
    for ( j=0; j<nfacesinbox; ++j )
    {
      ...
      if ( ! fg )
      {
        ...
        GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, ppoly);
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, polyg);
        lwfree(ids);
        _lwt_release_faces(faces, nfacesinbox);
      ...
      sp = GEOSPointOnSurface_r(lwcontext()->geos_ctx, fgg);
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, fgg);
      if ( ! sp )
      {
        GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, ppoly);
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, polyg);
      ...
      covers = GEOSPreparedCovers_r(lwcontext()->geos_ctx,  ppoly, sp );
      GEOSGeom_destroy_r(lwcontext()->geos_ctx, sp);
      if (covers == 2)
      {
        GEOSPreparedGeom_destroy_r(lwcontext()->geos_ctx, ppoly);
        GEOSGeom_destroy_r(lwcontext()->geos_ctx, polyg);
...
#define LWT_EDGERING_CLEAN(a) { \
  ...
  if ( (a)->genv ) { GEOSGeom_destroy_r(lwcontext()->geos_ctx, (a)->genv); (a)->genv = NULL; } \
}
...
#define LWT_EDGERING_ARRAY_CLEAN(a) { \
  ...
  if ( (a)->tree ) { \
    GEOSSTRtree_destroy_r(lwcontext()->geos_ctx,  (a)->tree ); \
    (a)->tree = NULL; \
  } \
}
                            ...
                            LWT_EDGERING_ARRAY *shells)
{
  ...
  if ( ! shells->tree )
  {
    ...
    LWDEBUG(1, "Building STRtree");
	  shells->tree = GEOSSTRtree_create_r(lwcontext()->geos_ctx, STRTREE_NODE_CAPACITY);
    if (shells->tree == NULL)
    {
    ...
    for (i=0; i<shells->size; ++i)
    {
      ...
      lwline_free(diag);
      GEOSSTRtree_insert_r(lwcontext()->geos_ctx, shells->tree, sring->genv, sring);
    }
  ...
  LWT_EDGERING_ARRAY_INIT(&candidates);
	GEOSSTRtree_query_r(lwcontext()->geos_ctx, shells->tree, ghole, &_lwt_AccumulateCanditates, &candidates);
  ...
  GEOSGeom_destroy_r(lwcontext()->geos_ctx, ghole);
