...
static LWGEOM_CONTEXT* default_contextor();
lwcontextor	lwcontext_var = (lwcontextor)default_contextor;
...
static LWGEOM_CONTEXT*
default_contextor()
{
	static LWGEOM_CONTEXT	default_context = { 0 };
...
lwgeom_set_handlers(lwallocator allocator, lwreallocator reallocator,
	        ...
	        lwreporter noticereporter, lwcontextor contextor) {
	...
	if (contextor) lwcontext_var = contextor;

}

void
lwgeom_set_json_handlers(lwallocator allocator, lwreallocator reallocator,
	lwfreeor freeor) {
	json_set_handlers(allocator, reallocator, freeor);
}
